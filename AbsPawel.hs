...
prExp (EBIf e1 e2 e3) = PP.text "if" PP.<+> prExp e1 PP.<+> PP.text "then" PP.$$ PP.nest 2 (prExp e2) PP.$$ PP.text "else" PP.$$ PP.nest 2 (prExp e3)
prExp (EBLet x tds e1 e2) = PP.text "let" PP.<+> PP.text (show x) PP.<+> PP.text "=" PP.<+> prExp e1 PP.<+> PP.text "in" PP.$$ PP.nest 2 (prExp e2)
