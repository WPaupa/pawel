...
mgu TInt (TFun (TInt) b)       =  throwError $ "TypeError, expected Int, got Int -> " ++ show b
mgu TInt (TFun (TFun t1 t2) b) =  do  s1 <- mgu (TFun t1 t2) b
                                      s2 <- mgu (apply s1 t1) (apply s1 t2)
                                      return (s1 `composeSubst` s2)
mgu TInt (TFun (TVar a) b)     =  mgu (TVar a) b
