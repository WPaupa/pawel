...
```
type List of a = Empty | Cons a (List a)
```
## Polimorfizm
...
Wyrażenie `if e1 then e2 else e3` jest de facto aliasem na funkcję `ifte e1 e2 e3`. Funkcja `ifte` jest przeładowana dla argumentów `e1` typu `Int`, ale można ją dodatkowo przeładowywać dla wybranych przez siebie argumentów. Mamy, że `ifte x` jest funkcją `λ x y . x` (czyli odpowiednikiem prawdy w tym języku), gdy x jest niezerowe oraz funkcją `λ x y . y` (czyli odpowiednikiem fałszu), gdy x jest zerowe. Wewnątrz języka zdefiniowane jest:
```
let eq x y = ifte (x - y)
op right == 4 = eq 
```
Oprócz tego do porównywania obiektów typu `Int` istnieją też operatory `>`, `>=`, `<=` i `<`, wyliczające się do `λ x y . x` lub `λ x y . y` w standardowy sposób.
## Działania arytmetyczne
Działania arytmetyczne +, *, - i / są domyślnie przeładowane dla typu `Int` oraz dla typów funkcyjnych:
- Działania arytmetyczne na obiektach typu Int są po prostu tymi działaniami na liczbach naturalnych
- Działania arytmetyczne pomiędzy dwiema funkcjami są określone "po wartościach", na przykład `(λ x . T) + (λ x . M) = λ x . T + M`
- Działania arytmetyczne pomiędzy funkcją a obiektem typu Int wstawiają ten obiekt pod abstrakcję funkcji, na przykład `(λ x . T) + M = λ x . T + M`
